...
class MainActivityX : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (preferenceHelper!!.isLogin) {
            val intent = Intent(this@MainActivityX, WelcomeActivity::class.java)
...
//        tvlogin = findViewById<View>(R.id.tvlogin) as TextView
        tvlogin!!.setOnClickListener {
            val intent = Intent(this@MainActivityX, LoginActivity::class.java)
            startActivity(intent)
            finish()
        }
    ...
    private fun parseRegData(response: String) {
        ...
        if (jsonObject.optString("status") == "true") {
            saveInfo(response)
            Toast.makeText(this@MainActivityX, "Registered Successfully!", Toast.LENGTH_SHORT).show()
            val intent = Intent(this@MainActivityX, WelcomeActivity::class.java)
            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK or Intent.FLAG_ACTIVITY_NEW_TASK)
            startActivity(intent)
            finish()
        } else {
            Toast.makeText(this@MainActivityX, jsonObject.getString("message"), Toast.LENGTH_SHORT).show()
        }
    }
